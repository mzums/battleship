...
fn play<U: UI>(ui: &mut U, game_state: &mut game::GameState) {
    while !game::GameState::check_if_end(game_state) {
        game_state.turn = game::Turn::Player;
        loop {
            ui.show_message("Your turn!");
            let (row, col) = ui.get_input(game_state);
            let msg = game::GameState::hit(&mut game_state.computers_board, (row, col), &mut game_state.computers_ships_lifes);
            ui.show_message(&msg);
        ...
        game_state.turn = game::Turn::Computer;
        ui.render(game_state);
        ...
        loop {
            ...
            ui.show_message(format!("\nComputer hit at ({}, {})!", last_move.0 + 1, last_move.1 + 1).as_str());
            ui.render(game_state);
            thread::sleep(time::Duration::from_millis(1000));
